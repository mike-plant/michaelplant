---
title: "Streetlight"
layout: "layouts/page.njk"
section: "streetlight"
contexts: ["Streetlight", "Tools", "ThirdPlaces"]
threads:
  - id: "findable"
    order: 1
    next: ["/streetlight/google-looks-for/"]
  - id: "how-i-work"
    order: 1
    next: ["/streetlight/audit/"]
bridges:
  - url: "/streetlight/audit/"
    label: "Start with the audit"
    because: "If you want the fix list and what matters first, this is the fastest path."
  - url: "/streetlight/google-looks-for/"
    label: "What Google is actually looking for"
    because: "This is the simplest way to understand why you’re not showing up."
  - url: "/tools/"
    label: "How I think about tools and systems"
    because: "Streetlight is practical, not gimmicky. This is why."
bubble:
  eligible: true
  contextKey: "Streetlight"
  lensScore: { "recent": 0.6, "proud": 0.7, "impact": 0.9, "workWithMe": 0.8 }
vibe:
  theme: "streetlight-night"
seo:
  description: "Streetlight helps people find your business right here by fixing the fundamentals that quietly block visibility on maps."
---

Most local businesses don’t have a marketing problem. They have a visibility problem.

People are already searching nearby. Maps decide who gets seen first.

## What Streetlight does

Streetlight helps you show up more reliably on Google Maps, Apple Maps, and local listings.

It’s not a long contract. It’s clarity, fixes that matter, and optional maintenance if you want it.

## Who it’s for

Local businesses that depend on discovery. People searching for what you offer, not your name.

## What to do next

If you want a quick, clear starting point, begin with a one-time audit.
